...
import { View, Animated, Platform, ActivityIndicator, StyleSheet, TouchableOpacity, Dimensions } from "react-native";
...
import { LINK_NOT_FOUND } from "../../../utils/Config";
    ...
    <Animatable.View delay={500} animation="fadeInDown">
      <View style={styles.topBar}>
        ...
        <Animated.View style={{ opacity: headerOpacity }}>
          <CustomText style={{ fontSize: 16, color: "#fff", fontWeight: "500" }}>{item.filename}</CustomText>
        </Animated.View>
        <View style={styles.shareIcon}>
          <ShareItem imageURL={item.url} title={item.filename} message={item.filename} />
        </View>
      </View>
      <Animated.View
        style={{
      ...
      ></Animated.View>
      <Animated.Image
        source={{ uri: item?.url || LINK_NOT_FOUND }}
        style={[
          styles.image,
          {
